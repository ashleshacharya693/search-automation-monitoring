...
# Fetch today's live match titles from OpenSearch
...
def test_live_match_dataset(query):
    """
    Checks that each live match title is returned in search results.
    Runs across all platforms — user_type is not a concern here,
    we just want to confirm the content is reachable at all.
    """
    ...
    for platform_name, platform_config in PLATFORMS.items():
        ...
        # Use a single neutral/anonymous user — we only care content exists
        user_type = "anonymous"
    ...
    # Fail only after all platforms are tested
